...
| `secugrpId` | 아래 코드표 | 증권 구분. 기본 `ST`(주권 보통주) |
...
> 부동산투자회사·ELW·ETN·ETF 등도 별도 코드가 있다(미확인). 필요하면
> 브라우저에서 해당 탭의 POST 폼을 캡처해 `secugrpId` 값을 확인하면 된다.
>
> **`detailType`은 `1`로 고정**한다. detailType=1이면 위 모든 secugrpId가 통일된
> 6컬럼(회사 단위)으로 나온다. (detailType=2는 종목 단위 + 종목명·ISIN 12자리로
> 스키마가 달라지므로 쓰지 않는다.) 수익증권 등 일부는 원본 데이터가 희소해
> 시장구분/상장일/종목코드가 비어 있는 행이 있을 수 있다.

## 시장 전체 집계 (`listed_issue_summary`)

같은 화면 상단의 **시장 전체 요약**은 별도 엔드포인트로 받는다
(`corpgeneral/listedIssueStatus.do`, `method=readListedIssueStatus`). 유가·코스닥·
코넥스 3개 표를 합쳐 `시장` 컬럼을 붙여 돌려준다.

```python
summ = fetch("listed_issue_summary", selDate="2026-07-21")
# 컬럼: 시장 · 구분 · 회사수 · 종목수 · 상장주식수(천주) · 자본금(백만원) · 시가총액(백만원)
```

> `selDate`가 상세(`listed_issue_status`)와 반대로 **대시 포함(YYYY-MM-DD)**을
> 요구한다. 어느 형식으로 줘도 내부에서 맞춘다.

**집계 ↔ 상세 정합성(검증됨).** 집계의 `회사수`는 상세 목록(`listed_issue_status`)의
행 수와 **정확히 일치**한다(2026-07-21, 코드가 매핑되는 12개 구분 전부 확인). 즉
상세 목록은 회사 단위이고, 집계의 `종목수`(우선주 등 포함)와는 다르다.
